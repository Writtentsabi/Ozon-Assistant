...
async function sendChat(prompt, base64Image = null, fileName = null) {
	...
	try {
		...
		if (base64Image) {
			// Καθαρισμός του Base64 prefix (data:image/jpeg;base64,) αν υπάρχει
		...
		// Ενημέρωση ιστορικού με δομή 'parts' (μόνο αν δεν υπάρχει σφάλμα)
		const userText = base64Image ? `${langData.file_prefix}${fileName}${langData.file_suffix}${prompt}`: prompt;
